...
fun squareBetweenExists(m: Int, n: Int): Boolean {
    for (k in sqrt(m.toDouble()).toInt()..sqrt(m.toDouble()).toInt() + 1)
...
fun hasDifferentDigits(n: Int): Boolean {
    ...
    for (i in 1..(digitNumber(n) - 1))
        if (x % 10 == a) x /= 10 else return true
    return false
}
